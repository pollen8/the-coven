// Taken from:  javascript-astar 0.4.1
...
interface ISearchOptions {
  heuristic?: (start: GridNode, end: GridNode) => GridNode['h'];
...
const astar = {
  ...
  *          astar.heuristics).
  */
  search: function(graph: IGraph, start: GridNode, end: GridNode, options?: ISearchOptions) {
    graph.cleanDirty();
  ...
  heuristics: {
    manhattan: function(pos0: IPoint, pos1: IPoint) {
      const d1 = Math.abs(pos1.x - pos0.x);
      const d2 = Math.abs(pos1.y - pos0.y);
      return d1 + d2;
    },
    diagonal: function(pos0: IPoint, pos1: IPoint) {
      const D = 1;
...
class Graph {
  ...
  constructor(gridIn: TWeights, options: IGraphOptions = {}) {
    this.nodes = [];
  ...
  toString() {
    ...
    let rowDebug; let row; let y; let l;
  ...
  bubbleUp(n: number) {
    ...
    while (true) {
      ...
      if (child2N < length) {
        ...
        if (child2Score < Number(swap === null ? elemScore : child1Score)) {
          swap = child2N;
        }
      }
